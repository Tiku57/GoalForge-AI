...
`;
      ...
      // We no longer need to sanitize PDF chars, just use raw markdown
      const markdownContent = frontmatter + resultData;

      const blob = new Blob([markdownContent], { type: 'text/markdown;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `${title.toString().replace(/\s+/g, '_')}.md`;
            ...
            {resultData ? (
              <div className="bg-[#0f1115] rounded-xl p-6 border border-white/10 shadow-inner">
                ...
                <div className="mt-6 flex gap-3 pt-6 border-t border-white/10">
                  <button onClick={handleExportMarkdown} disabled={isExporting} className="flex-1 bg-white/5 hover:bg-white/10 border border-white/10 transition-colors py-2.5 rounded-lg text-xs font-semibold text-white flex items-center justify-center gap-2">
                    {isExporting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <FilePlus className="w-3.5 h-3.5" />} 
